(() => {
	...
	firebase.auth().onAuthStateChanged(user => {
		...
		document.getElementById("profile-name").innerHTML = firebase.auth().currentUser.displayName;
	});
	...
	tweetsDB.on('child_added', data => {
		...
		cont.setAttribute("id", data.key);
		cont.classList.add("post");
		
		cont.innerHTML = post(data);
		let reacts = cont.querySelectorAll('.post-reaction');
	...
	tweetsDB.on('child_changed', data => {
		let tweet = document.getElementById(data.key);
		tweet.querySelector('.post-likes').innerText = data.val().likes;
		tweet.querySelector('.post-dislikes').innerText = data.val().dislikes;
	});
